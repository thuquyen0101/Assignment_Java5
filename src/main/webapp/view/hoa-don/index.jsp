                        ...
                        <table class="table table-hover">
                            ...
                            <c:forEach items="${listHD}" var="x" varStatus="i">
                                <tr>
                                    ...
                                    <td>${x.id}</td>
                                    <td>${x.ma}</td>
                                    <td>${x.khachHang.ma}</td>
                                    <td>${x.ngayTao}</td>
                                    <td>${x.ngayThanhToan}</td>
                                    <td>
                                        <a class="btn btn-success" href="/hoa-don/detail?idHD=${x.id}">Xem chi tiết </a>
                                    </td>
                                </tr>
                            </c:forEach>
                            </tbody>
                        ...
                        <ul class="pagination">
                            <c:if test="${totalPage > 0}">
                                <c:if test="${currentPage > 0}">
                                    <li class="page-item">
                                        <a class="page-link"
                                           href="/hoa-don/hien-thi?pageNumber=${currentPage - 1}">Previous</a>
                                    </li>
                                </c:if>
                                <c:if test="${currentPage == 0}">
                                ...
                                <c:forEach begin="0" end="${totalPage -1}" var="x">
                                    <li class="page-item"><a class="page-link"
                                                             href="/hoa-don/hien-thi?pageNumber=${x}">${x+1}</a></li>
                                </c:forEach>
                                <c:if test="${currentPage < (totalPage -1)}">
                                    <li class="page-item">
                                        <a class="page-link"
                                           href="/hoa-don/hien-thi?pageNumber=${currentPage + 1}">Next</a>
                                    </li>
                                </c:if>
                                <c:if test="${currentPage == (totalPage -1)}">
                                    <li class="page-item disabled">
